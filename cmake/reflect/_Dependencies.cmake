...
#
# Generated from sourcetree: mulle-url;no-all-load,no-import,no-singlephase;
# Disable with: `mulle-sourcetree mark mulle-url no-header`
#
if( NOT MULLE_URL_LIBRARY)
   find_library( MULLE_URL_LIBRARY NAMES ${CMAKE_STATIC_LIBRARY_PREFIX}mulle-url${CMAKE_STATIC_LIBRARY_SUFFIX} mulle-url NO_CMAKE_SYSTEM_PATH NO_SYSTEM_ENVIRONMENT_PATH)
   message( STATUS "MULLE_URL_LIBRARY is ${MULLE_URL_LIBRARY}")
   #
   # The order looks ascending, but due to the way this file is read
   # it ends up being descending, which is what we need.
   #
   if( MULLE_URL_LIBRARY)
      #
      # Add MULLE_URL_LIBRARY to DEPENDENCY_LIBRARIES list.
      # Disable with: `mulle-sourcetree mark mulle-url no-cmakeadd`
      #
      ...
      #
      # Inherit ObjC loader and link dependency info.
      # Disable with: `mulle-sourcetree mark mulle-url no-cmakeinherit`
      #
      # // temporarily expand CMAKE_MODULE_PATH
      get_filename_component( _TMP_MULLE_URL_ROOT "${MULLE_URL_LIBRARY}" DIRECTORY)
      get_filename_component( _TMP_MULLE_URL_ROOT "${_TMP_MULLE_URL_ROOT}" DIRECTORY)
      #
      #
      # Search for "DependenciesAndLibraries.cmake" to include.
      # Disable with: `mulle-sourcetree mark mulle-url no-cmakedependency`
      #
      foreach( _TMP_MULLE_URL_NAME "mulle-url")
         set( _TMP_MULLE_URL_DIR "${_TMP_MULLE_URL_ROOT}/include/${_TMP_MULLE_URL_NAME}/cmake")
         # use explicit path to avoid "surprises"
         if( EXISTS "${_TMP_MULLE_URL_DIR}/DependenciesAndLibraries.cmake")
            unset( MULLE_URL_DEFINITIONS)
            list( INSERT CMAKE_MODULE_PATH 0 "${_TMP_MULLE_URL_DIR}")
            # we only want top level INHERIT_OBJC_LOADERS, so disable them
            if( NOT NO_INHERIT_OBJC_LOADERS)
               set( NO_INHERIT_OBJC_LOADERS OFF)
            endif()
            ...
            set( NO_INHERIT_OBJC_LOADERS ON)
            #
            include( "${_TMP_MULLE_URL_DIR}/DependenciesAndLibraries.cmake")
            #
            list( GET _TMP_INHERIT_OBJC_LOADERS -1 NO_INHERIT_OBJC_LOADERS)
            list( REMOVE_AT _TMP_INHERIT_OBJC_LOADERS -1)
            #
            list( REMOVE_ITEM CMAKE_MODULE_PATH "${_TMP_MULLE_URL_DIR}")
            set( INHERITED_DEFINITIONS
               ${INHERITED_DEFINITIONS}
               ${MULLE_URL_DEFINITIONS}
               CACHE INTERNAL "need to cache this"
            )
            break()
         else()
            message( STATUS "${_TMP_MULLE_URL_DIR}/DependenciesAndLibraries.cmake not found")
         endif()
      endforeach()
   else()
      message( FATAL_ERROR "MULLE_URL_LIBRARY was not found")
   endif()
endif()
